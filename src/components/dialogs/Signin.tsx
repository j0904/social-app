...
import {createWallet, loadWallet} from '#/screens/wallet/WalletHelper'
...
function SigninDialogInner({}: {control: Dialog.DialogOuterProps['control']}) {
  ...
  const handleGenerateWallet = React.useCallback(() => {
    // For now using a placeholder mnemonic since generateWalletMnemonic was removed
    const mnemonic =
  ...
  const handleLoadWallet = React.useCallback(async () => {
    try {
      ...
      // For compatibility, create a wallet file using hdwallet module
      // The file content will be loaded as an encrypted wallet file
      const walletFile = await loadWallet(fileContent, mnemonic) // Using content as password for demo
      setWalletInfo(
        `Loaded wallet\nAddress: ${walletFile.wallet.address}\nUser: ${walletFile.credentials.user}\nPassword: ${walletFile.credentials.password}`,
      )
    } catch (e: any) {
